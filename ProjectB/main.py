...
def update_hover_graph(clickData, stored_dataframe):
    ...
    fig.update_layout(
        ...
        title_x=0.5,
        yaxis=dict(range=[0, dff["Life expectancy at birth, total (years)"].max() + 10])
    )
    ...
    # Generated with Grok 3 prompt: Okay, what if I want to draw a line starting at 2000 at 66 to 73 in the year 2022
    # as my annotation? How would I do that?
    fig.add_shape(
        type="line",
        x0=2000,  # Starting x (year 2000)
        y0=66,  # Starting y (life expectancy 66)
        x1=2022,  # Ending x (year 2022)
        y1=73,  # Ending y (life expectancy 73)
        line=dict(
            color="red",  # Line color
            width=2,  # Line thickness
            dash="dash"  # Line style (optional: "solid", "dot", "dash", etc.)
        )
    )
    ...
    fig.add_annotation(
        x=2015,
        y=60,
        text="Worldwide Average Life Expectancy",
        showarrow=False,  # No arrow since the text is the annotation
        yshift=0,  # Move text slightly above the line
        font=dict(color="red"),  # Match text color to line
